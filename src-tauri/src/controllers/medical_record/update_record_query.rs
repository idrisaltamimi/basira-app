...
#[tokio::main]
pub async fn update_record_query(data: CreateRecordData) -> Result<()> {
    let db = database().await?;

    let sql = format!(
        r#"
        UPDATE record SET did_surgery = $did_surgery, 
...
#[tauri::command]
pub fn update_record(data: CreateRecordData) {
    let _record = update_record_query(data);
}
